...
/* return randombytes in buf with  lenth of buflen */
void randombytes_forcake(unsigned char *buf,size_t buflen)
{
  size_t d = 0;
  int r;
...
/*return a big number(secp256k1_scalar) with 256 bits  */
void random_scalar_generation(secp256k1_scalar *num) {
    do {
        unsigned char b32[32];
        int overflow = 0;
        randombytes_forcake(b32, 32);
...
/*return a big number(secp256k1_scalar) with 128 bits  */
void random_scalar_generation_halfq(secp256k1_scalar *num) {
    do {
        unsigned char b16[16]; 
        int overflow = 0;
        randombytes_forcake(b16, 16);


        secp256k1_scalar_set_b32_halfq(num, b16, &overflow);
        if (overflow || secp256k1_scalar_is_zero(num)) {
            continue;
        }
        break;
